...
{
    public class StopBroadcastSerializeAction : SerializeAction
    {
        ...
        {
            var response = await _fuelPumpRestClient.StopBroadcast();
            var data = await response.Content.ReadAsStringAsync();

            switch (response.StatusCode)
            {
                case HttpStatusCode.OK:
                    var contract = new DeSerializer().MapBool(data);
                    return contract;
                default:
