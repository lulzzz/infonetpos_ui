...
namespace MyToolkit.Controls
{
    ...
    public class MtListBox : ListBox
    {
            ...
            ItemContainerStyle = (Style)XamlReader.Load(
                @"<Style TargetType=""ListBoxItem"" xmlns=""http://schemas.microsoft.com/winfx/2006/xaml/presentation"">
                    <Setter Property=""Template"">
                        <Setter.Value>
                            <ControlTemplate>
                                <ContentPresenter HorizontalAlignment=""Stretch"" Margin="0" Padding="0" VerticalAlignment=""Stretch""/>
                            </ControlTemplate>
                        </Setter.Value>
                    </Setter>
                </Style>");
        ...
        private void CreateVerticalPartitions()
        {
            var canvas = (Canvas)GetTemplateChild("Canvas");
            var grid = this.GetVisualParentOfType<DataGrid>();
            ...
            if (grid != null && canvas != null)
            {
                ...
                // Draw partitions only if width is star based

                if (grid.Columns.Any(x => !x.Width.IsStar))
                {
                    return;
                }

                var totalParts = grid.Columns
                    .Sum(x => x.Width.Value);
                var shift = 0d;
                foreach (var column in grid.Columns)
                {
                    shift += column.Width.Value * width / totalParts;
                    var line = new Line();
                    line.Stroke = grid.HeaderBackground;
                    line.X1 = shift;
                    line.X2 = shift;
                    line.Y1 = 0;
                    line.Y2 = height;
        ...
        {
            if (_scrollViewer != null && _contentGrid != null)
            {
                var itemsPresenter = (ItemsPresenter)_contentGrid.GetVisualChild(0);
        ...
        {
            _scrollViewer.ChangeView(_scrollViewer.HorizontalOffset, _scrollViewer.ScrollableHeight, _scrollViewer.ZoomFactor);
        }
    }
